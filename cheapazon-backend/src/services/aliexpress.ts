...
import { extractKeywords } from './gemini';
...
export const searchAliExpress = async (product: AmazonProduct): Promise<AliExpressProduct | null> => {
    ...
    if (product.title.length > 100) {
        console.log(`Title is too long, using Gemini to extract keywords`);
        searchKeywords = await extractKeywords(product.title);
        console.log(`Gemini optimized keywords: ${searchKeywords}`);
    } else {
        console.log(`Title is short, using original title: ${searchKeywords}`);
    }
